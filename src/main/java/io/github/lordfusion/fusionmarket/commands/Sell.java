...
public class Sell
{
    ...
    public static void run(Player player, String[] args)
    {
        ...
        if (shop.getPrice() > 0) {
            BaseCommand.commandFailure(player, "'" + shop.getUniqueId() + "' is not buying.");
            return;
        }
        ...
        if (args.length == 2) {
            ...
            if (shop.isAdminShop())
                shop.adminShopBuyFrom(player, Integer.parseInt(args[1]));
            else
                shop.buyFrom(player, Integer.parseInt(args[1]));
        } else
