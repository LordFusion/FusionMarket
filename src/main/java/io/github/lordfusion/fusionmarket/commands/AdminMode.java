...
public class AdminMode
{
    ...
    public static void run(CommandSender sender, String[] args)
    {
        ...
        boolean newValue;
        if (args.length == 0) {
            newValue = !FusionMarket.getInstance().getDataManager().isAdminMode(player);
        } else {
            newValue = Boolean.parseBoolean(args[0]);
        }
        FusionMarket.getInstance().getDataManager().setAdminMode(player, newValue);
        FusionMarket.sendConsoleInfo("Admin mode set to " + newValue + " for " + player.getName() + ".");
        
        TextComponent msg1 = new TextComponent("Admin mode has been ");
